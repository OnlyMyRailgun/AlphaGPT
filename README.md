...
1.  **准备环境与数据库**:
    ...
    cp .env.example .env
    ```
    编辑 `.env` 文件，确保 `DB_HOST=localhost`，并设置数据源：
    ```bash
    # 方案一：Birdeye（需要 API Key）
    DATA_PROVIDER=birdeye
    BIRDEYE_API_KEY=your_birdeye_api_key_here

    # 方案二：DexScreener（公开接口，可不填 Birdeye Key）
...
```
说明：
- 当 `DATA_PROVIDER=birdeye` 时，启动前会校验 `BIRDEYE_API_KEY`
- 当 `DATA_PROVIDER=dexscreener` 时，不需要 `BIRDEYE_API_KEY`
...
`data_pipeline` 现在支持两种数据源，通过环境变量显式切换：

- `DATA_PROVIDER=birdeye`
  - 优点：字段更完整，和当前主流程契合度更高
  - 要求：必须配置 `BIRDEYE_API_KEY`
- `DATA_PROVIDER=dexscreener`
  - 优点：公开接口可直接使用，更适合快速跑通
  - 要求：不需要 `BIRDEYE_API_KEY`

两种 provider 都会被规范化成统一的 token 与 OHLCV 入库结构，因此数据库层和后续模型层无需改动。
